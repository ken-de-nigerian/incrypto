<script setup lang="ts">
    ...
    const props = defineProps<{
        ...
        commoditiesPairs: Array<CryptoPair>;
        commodities: Array<Trade>;
        auth: {
            user: {
                profile: UserProfile;
                first_name: string;
    ...
    const fetchPairData = async (symbol: string): Promise<CryptoPair | null> => {
        ...
        try {
            ...
            const params = new URLSearchParams({ category: 'commodities' });
    ...
    onMounted(async () => {
        ...
        try {
            ...
            if (persistedSymbol) {
                initialPair = props.commoditiesPairs.find(p => p.symbol === persistedSymbol);
            }

            if (!initialPair) {
                initialPair = props.commoditiesPairs.find(p => p.price) || props.commoditiesPairs[0];
            }
...
<template>
    <Head title="Commodities Trading" />
